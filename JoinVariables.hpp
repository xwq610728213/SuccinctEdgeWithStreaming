...
public:
    ...
    vector<long> get_data_variables_indexes(vector<string> &list_data_variables){
        vector<long> temp_result;
        for(auto i = 0; i < list_data_variables.size(); ++i){
            auto index = get_data_index(list_data_variables[i]);
            if(index >= 0){
                temp_result.push_back(index);
            }else{
                cerr << list_data_variables[i] << " is not a data type variable!" << endl;
            }
        }
        return temp_result;
    }

    long get_data_variables_indexes(string str){
        return get_data_index(str);
    }

    long get_data_index(string& str){
        for(auto i = 0; i < var_str_datas.size(); ++i){
            if(var_str_datas[i] == str){
                return i;
            }
        }
        return -1;
